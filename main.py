...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        model.train()
        for ii, batch in tqdm(enumerate(train_dataloader), desc=f"train(epoch={epoch})", total=len(train_dataloader)):
            ...
            if ii % opt.print_freq == 0:
                logger.info('epoch:%04d, loss:%f' % (epoch, loss.item()))
    ...
    for index, batch in enumerate(valid_dataloader):
        ...
        if index % 5 == 0:
            logger.info(tokenizer.convert_ids_to_tokens([i.item() for i in x[0].cpu() if i.item() > 0]))
            length = [id2tags[i.item()] for i in y[0].cpu() if i.item() > 0]
            logger.info(length)
            logger.info([id2tags[i] for i in predict[0][:len(length)]])
